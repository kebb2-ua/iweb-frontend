...
<template>
  <v-container>
    ...
    <!-- Loading -->
    <v-progress-circular v-if="loading" indeterminate color="blue" class="my-4" />
        ...
        <v-col v-for="(user, index) in users" :key="index" cols="12" sm="6" md="4">
          <v-card class="mb-4">
            ...
            <v-card-text>
              ...
              <v-select :items="availableRoles" v-model="user.selectedRole" label="Rol"
                @update:modelValue="val => changeUserRole(user.email, val)" />
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
